...
from app.mongodb import init_or_get_conversation
...
async def test_init_or_get_conversation_existing():
    ...
    with patch("app.mongodb.get_conversation_by_id", AsyncMock(return_value=existing_conversation)):
...
async def test_init_or_get_conversation_non_existing():
    ...
    with patch("app.mongodb.get_conversation_by_id", AsyncMock(return_value=None)):
        result = await init_or_get_conversation(query)
...
async def test_init_or_get_conversation_error():
    ...
    with patch("app.mongodb.get_conversation_by_id", AsyncMock(side_effect=Exception("Database error"))):
