...
def get_attribute_and_explanation_object(attribute: str, explanation: str, label: str, percentage: str):
    attribute = clean_string(attribute.strip())
    explanation = explanation.strip()
    label = label.strip()
    percentage = percentage.strip().rstrip('.')
    evaluation = {"label": label, "percentage": percentage}
    return {"attribute": attribute, "explanation": explanation, "evaluation": evaluation}

def get_attribute_and_explanation_object_array(input: str):
    """
    ...
    for line in input.split('\n'):
        ...
        attribute, explanation, evaluation = parts
        eval_parts = [part.replace("{", "").replace("}", "").strip() for part in evaluation.split(':')]
        if len(eval_parts) != 2:
            raise ValueError(f"Invalid evaluation format: {evaluation}")
